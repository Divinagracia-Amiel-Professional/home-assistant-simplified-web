...
const DevicesCards = (props: any) => {
  ...
  const iconSizeLogic = windowDimensions.width >= 1000 ? 100 : windowDimensions.width >= 360 ? 75 : 50;
...
const DeviceCard = (props: any) => {
  ...
  return (
    ...
    >
      <p className='details-header'>{deviceInfo.name}</p>
      {getIcon({
        ...defaultIconParams,
        ...iconParams,
      })}
      <div className='device-value-container'>
        <p className='device-value-text'>{deviceInfo.value}</p>
        <p className='details-header device-unit'>kWh</p>
      </div>
    </div>
  );
};

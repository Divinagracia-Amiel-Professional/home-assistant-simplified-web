...
const groupByHours = async (timeframe: TimeFrame, data: any[], name: string, differenceInDays: number) => {
  ...
  const trimMonth = removeEmpty.map((month, index, arr) => {
    ...
    if (arrLength !== 1) {
      if (index === 0) {
        return month?.filter(dayItem => {
          return dayItem[0] >= timeFrameDates.start;
        });
      } else {
        return month?.filter(dayItem => {
          return dayItem[0] <= timeFrameDates.end;
        });
      }
    } else {
      return month?.filter(dayItem => {
        return dayItem[0] >= timeFrameDates.start && dayItem[0] <= timeFrameDates.end;
      });
    }
  });
  ...
  const flatMonth = trimMonth.flatMap(month => {
    const days = month.map(dayItem => {
      ...
      const toArray = Object.entries(dayItem[1]).map(([key, val]) => [key, val]);
  ...
  const computeKwh = filledDayArr.map((item, index, arr) => {
    ...
    if (item[1].length === 1 && prevState === null) {
      //firstItemCase is an hourInterval
      ...
      //when hourArray has 5 minute intervals
      let accumulator = 0;
      let firstItemIndex = 0;
      if (prevState === null) {
        prevState = nearestPrevData;
...
const groupByDays = async (timeframe: TimeFrame, data: any[], name: string) => {
  ...
  const { grouped: groupedByYears, nearestPrevData } = groupByYearsObject(data, timeframe, name);
  ...
  const trimMonth = removeEmpty.map((month, index, arr) => {
    ...
    return month?.filter(dayItem => {
      return dayItem[0].getTime() >= timeFrameTime.start && dayItem[0].getTime() < timeFrameTime.end;
    });
  });
...
const groupByMonths = async (timeframe: TimeFrame, data: any[], name: string, isSummary: boolean) => {
  ...
  const { grouped: groupedByYears, nearestPrevData } = await groupByYearsObject(data, timeframe, name);
      ...
      Object.entries(parsed).forEach(async ([key, item]) => {
        ...
        const hourDayTimeFrame: TimeFrame = {
          start: parseTimestamp(timeFrame.start),
            ...
            ? parseTimestamp(parseTimestamp(timeFrame.end).getTime() + 1000 * 60 * 60 * 24)
            : parseTimestamp(timeFrame.start),
        };
